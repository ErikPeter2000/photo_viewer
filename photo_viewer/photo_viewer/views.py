...
class ImageDetailView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        photographer_name = f"{image.owner.first_name} {image.owner.last_name} ({image.owner.username})"
        return render(request, self.template_name, {'image': image, 'uploaded_by': str(photographer_name), 'uploaded_at': str(image.date_created)})
...
def upload_images(request, album_id):
    ...
    messages.success(request, f"{len(images)} Image{'s' if len(images) > 1 else ''} uploaded successfully.")
